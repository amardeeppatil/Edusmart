...
public class RequestHandlerController {
	...
	public String facultyRequestToCenter(@ModelAttribute("facultyData") FacultyCenterRequest facultyCenterRequest,HttpServletRequest request){
		...
		sendRequestDetailsList=(List)CommonController.getAllObjectsByQuery("from SendRequestDetails where fromWho='"+user.getUserId()+"'  and  toWhom='"+userProfileTB.getUserId()+"'");	
	...
	@RequestMapping("academyRequestAsscociation")
	@ResponseBody
	public String academyRequestToFaculty(@ModelAttribute("getFacultyData") FacultyCenterRequest facultyCenterRequest,HttpServletRequest request){
		try{
		UserProfileTB user = UserProfileTB.class.cast(request.getSession().getAttribute("USER"));
		...
		sendRequestDetails.setToWhom(user.getUserId());
		sendRequestDetails.setFromWho(userProfileTB.getUserId());
		sendRequestDetails.setStatus("Pending");
		sendRequestDetails.setDate(dateFormatWithTime.format(calendar.getTime()));
		sendRequestDetails.setFacultyTb(faculty);
		sendRequestDetails.setAcademyTB(academyTB);
