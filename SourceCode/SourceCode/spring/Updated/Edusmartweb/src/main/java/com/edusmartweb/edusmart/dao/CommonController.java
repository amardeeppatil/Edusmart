...
public class CommonController extends com.edusmartweb.edusmart.utils.HibernateUtil {
	...
	public static String saveAllFacultySubjects(int courseID,UserProfileTB user,FacultyTb facultyTb,String SubjectArray[]){
		 ...
		 try{
			...
			int indexValue=0;
			while(SubjectArray.length>indexValue)
			  { 
				FacultyDetailsTB facultyDetailsTB=new FacultyDetailsTB();
				System.out.println(SubjectArray[indexValue]);
				List<SubjectLevelTB> subjectLevelTBList=(List)CommonController.getAllObjects(SubjectLevelTB.class,"subjectName='"+SubjectArray[indexValue]+"'");
			if(subjectLevelTBList.size()>0)
			  {
				subjectLevelTBset.add(subjectLevelTBList.get(0));
				facultyDetailsTB.setFacultyID(facultyTb.getFacultyId());
				facultyDetailsTB.setCourseID(courseTB.getCourseId());
				facultyDetailsTB.setSubjectLevelID(subjectLevelTBList.get(0).getSubjectId());
			//facultyDetailsTB.setUserDetailsID(user.getUserId());
			    CommonController.saveOrUpdateObject(facultyDetailsTB);       
			  }
			indexValue++;
			}
