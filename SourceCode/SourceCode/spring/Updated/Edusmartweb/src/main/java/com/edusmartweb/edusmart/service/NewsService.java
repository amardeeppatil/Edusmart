...
import com.edusmartweb.edusmart.model.EventsTB;
...
public class NewsService {
	...
	{    
		...
		//query= "newsTB.getEntryBy() =user.getUserType()";
				...
				if (pastNewsList2.size() > 0) {
					...
					userSession.setAttribute("newsList", newsList);
		}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void eventDisplay(Model model,HttpServletRequest request){
		
		try{
			List<EventsTB> eventsList = (List) CommonController.getAllObjects("from EventsTB");
	        HttpSession userSession=request.getSession(false);
	     	userSession.setAttribute("eventsList", eventsList);
